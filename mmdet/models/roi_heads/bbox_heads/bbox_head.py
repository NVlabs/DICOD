    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                pos_n = pos_mask.sum()
                losses['acc_pos'] = accuracy(cls_score[pos_mask], labels[pos_mask])
                losses['acc_neg'] = accuracy(cls_score[~pos_mask], labels[~pos_mask])
                neg_labels = labels[pos_mask].new_zeros(pos_n)
                neg_labels = neg_labels.new_full((pos_n, ),
                                                  self.num_classes,
                                                 dtype=torch.long)
                losses['pos_to_neg'] = accuracy(cls_score[pos_mask], neg_labels)
